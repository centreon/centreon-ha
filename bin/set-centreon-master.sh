...
if [[ "$VIP_USED" == 1 ]] ; then
    ...
    if [[ "$NEW_MASTER_HOSTNAME" != "$CURRENT_MASTER_HOSTNAME" ]] ; then
        ...
        [[ "$DEBUG" ]] && set -x
        VIP_SHUTDOWN_OUTPUT=$(sudo -u centreon ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${CURRENT_MASTER_HOSTNAME} -- sudo nmcli con mod "${CENTRAL_VIP_IFNAME}" -ipv4.addresses "${CENTRAL_VIP_IPADDR}/${CENTRAL_VIP_CIDR_NETMASK}" && sudo -u centreon ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${CURRENT_MASTER_HOSTNAME} -- sudo nmcli connection up "${CENTRAL_VIP_IFNAME}" 2>&1)
        VIP_SHUTDOWN_RC=$?
        [[ "$DEBUG" ]] && set +x
...
[[ "$DEBUG" ]] && set -x
VIP_START_OUTPUT=$(sudo -u centreon ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${NEW_MASTER_IPADDR} -- sudo nmcli con mod "${CENTRAL_VIP_IFNAME}" +ipv4.addresses "${CENTRAL_VIP_IPADDR}/${CENTRAL_VIP_CIDR_NETMASK}" && sudo -u centreon ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${NEW_MASTER_IPADDR} -- sudo nmcli connection up "${CENTRAL_VIP_IFNAME}" 2>&1)
VIP_START_RC=$?
[[ "$DEBUG" ]] && set +x
